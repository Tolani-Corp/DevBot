# DevBot Commercial License

Copyright (c) 2024-2026 Tolani Labs. All Rights Reserved.

Additional grant for education:

- Authorized Tolani Labs education users may also use DevBot under the perpetual education grant in `EDUCATION-LICENSE.md`.

## License Grant

This software is licensed, not sold. Subject to the terms of this license, Tolani Labs grants you a limited, non-exclusive, non-transferable license to:

### Free Tier
- Use DevBot for personal and evaluation purposes
- Process up to 50 tasks per month
- Connect up to 2 repositories

### Pro Tier ($29/month)
- Use DevBot for commercial purposes
- Process up to 500 tasks per month
- Connect up to 10 repositories
- PR review automation
- Custom bot naming

### Team Tier ($99/month)
- All Pro features
- Process up to 2,000 tasks per month
- Connect up to 50 repositories
- Health scanning
- Team analytics
- Approval workflows

### Enterprise Tier (Custom pricing)
- Unlimited tasks and repositories
- Multi-agent orchestration
- Self-hosted deployment rights
- Priority support
- Custom integrations

## Restrictions

You may NOT:
- Redistribute, sublicense, or resell this software
- Remove or alter any proprietary notices
- Use this software to build a competing product
- Reverse engineer the software except as permitted by law

## Self-Hosted Deployment

Self-hosted deployment requires an active Enterprise license. The Docker image and source code are provided for deployment purposes only.

## Supply Chain Attestation

All releases include SLSA Level 3 provenance attestation. Verify releases using:
```bash
slsa-verifier verify-artifact debo-*.tar.gz --provenance-path debo-*.intoto.jsonl --source-uri github.com/Tolani-Corp/DevBot
```

## Support

- Free/Pro: Community support via GitHub Issues
- Team: Email support (48h response)
- Enterprise: Priority support (4h response)

## Warranty Disclaimer

THIS SOFTWARE IS PROVIDED "AS IS" WITHOUT WARRANTY OF ANY KIND. TOLANI LABS DISCLAIMS ALL WARRANTIES, EXPRESS OR IMPLIED.

## Contact

- Sales: sales@tolani.dev
- Support: support@tolani.dev
- Security: security@tolani.dev
