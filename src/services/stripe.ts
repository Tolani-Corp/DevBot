...
export const PRICE_TO_TIER: Record<string, TierName> = {
  ...
  [process.env.STRIPE_PRICE_ENTERPRISE_MONTHLY ?? "__enterprise__"]:
    "enterprise",
};
...
export async function applySubscriptionToWorkspace(
  ...
  const periodEnd = new Date((primaryItem?.current_period_end ?? 0) * 1000);
  ...
  await db
    ...
    .set({
      ...
      billingStatus: subscription.status as
        | "active"
        | "past_due"
        | "canceled"
        | "trialing",
  ...
  console.log(
    `[stripe] Workspace ${workspaceId} → tier: ${tier} (${subscription.status})`,
  );
}
