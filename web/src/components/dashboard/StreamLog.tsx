          ...
          <div key={i} className="flex gap-2 animate-in fade-in slide-in-from-bottom-1 duration-200">
            ...
            {log.type === 'task:started' && (
              <div className="text-yellow-400">
                ...
                Started Task: {log.data?.id ?? 'Unknown'} - {log.data?.description ?? renderPayload(log.data)}
              </div>
            )}
            {log.type === 'task:completed' && (
              <div className="text-green-400">
                <span className="mr-2">DONE</span>
                Completed: {log.data?.taskId ?? log.data?.id ?? 'Unknown'}
              </div>
            )}
            {log.type === 'log' && (
              <div className="text-slate-300">
                <span className="text-blue-500 mr-2">INFO</span>
                {log.message ?? renderPayload(log.data)}
              </div>
            )}
            {log.type === 'connected' && (
