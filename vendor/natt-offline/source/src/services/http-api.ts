...
import {
  ...
  type GrantWatchRunInput,
} from "@/agents/grant-watch";
...
import { getGovernanceProofOverview } from "@/services/delivery-governance";
import {
  deployConvoy,
  dispatchTask,
  forgetMemory,
...
function formatPolicyResponse(
  snapshot: Awaited<ReturnType<typeof getWorkspacePolicySnapshot>>,
) {
  return {
...
export async function handleApiRequest(
  ...
  try {
    ...
    if (method === "POST" && path === "/api/health-scan") {
      ...
      try {
        ...
        sendJson(res, 200, {
          ...
          critical: report.criticalIssues.filter(
            (issue) => issue.severity === "critical",
          ).length,
          high: flattened.filter((issue) => issue.severity === "high").length,
          medium: flattened.filter((issue) => issue.severity === "medium")
            .length,
    ...
    if (method === "GET" && path === "/api/governance/proof") {
      sendJson(res, 200, getGovernanceProofOverview());
      return true;
    }

    if (method === "POST" && path === "/api/funding/grant-watch") {
      const body = await readJson<GrantWatchRunInput & { persist?: boolean }>(
        req,
      );
    ...
    if (method === "GET" && path === "/api/journeys") {
      const journeys = (
        await listJourneys(url.searchParams.get("workspaceId") ?? undefined)
      ).map((journey) => ({
        id: journey.id,
        name: journey.goal,
        title: journey.goal,
        ...
        progress:
          journey.status === "completed"
            ? 100
            : journey.currentStage === "review"
              ? 80
              : journey.currentStage === "execution"
                ? 55
                : 20,
    ...
    const journeyReflectMatch =
      method === "POST"
        ? path.match(/^\/api\/journeys\/([^/]+)\/reflect$/)
        : null;
    if (journeyReflectMatch) {
      sendJson(
        res,
        200,
        await runReflectionGeneration(journeyReflectMatch[1]!),
      );
      return true;
    }

    const journeyMatch =
      method === "GET" ? path.match(/^\/api\/journeys\/([^/]+)$/) : null;
    if (journeyMatch) {
    ...
    if (
      (method === "POST" &&
        (path === "/api/memory/query" || path === "/api/memory/search")) ||
    ...
    if (method === "POST" && path === "/api/memory/forget") {
      ...
      const result = await forgetMemory({
        workspaceId:
          body.workspaceId ??
          (body.target?.startsWith("workspace:")
            ? body.target.slice("workspace:".length)
            : undefined),
        journeyId:
          body.journeyId ??
          (body.target?.startsWith("journey:")
            ? body.target.slice("journey:".length)
            : undefined),
        reason: body.reason,
      });
      sendJson(res, 200, {
        ...
        message:
          result.updatedJourneys > 0
            ? "Journey summaries cleared."
            : "Memory events removed.",
      });
      return true;
    }

    const deleteMemoryMatch =
      method === "DELETE" ? path.match(/^\/api\/memory\/([^/]+)$/) : null;
    if (deleteMemoryMatch) {
      const target = decodeURIComponent(deleteMemoryMatch[1]!);
      const result = await forgetMemory({
        workspaceId: target.startsWith("workspace:")
          ? target.slice("workspace:".length)
          : undefined,
        journeyId: target.startsWith("journey:")
          ? target.slice("journey:".length)
          : undefined,
      });
      sendJson(res, 200, {
        target,
    ...
    if (
      ...
      const updatedWorkspace = await updateWorkspacePolicy({
        ...
        memoryPolicy: {
          ...
          retentionDays:
            body.memoryRetentionDays ??
            (body.memoryPolicy as { retentionDays?: number } | undefined)
              ?.retentionDays,
        } as never,
        approvalPolicy: {
      ...
      sendJson(
        ...
        formatPolicyResponse(
          await getWorkspacePolicySnapshot(updatedWorkspace.id),
        ),
      );
      return true;
    }
    ...
    if (
      method === "GET" &&
      (path === "/api/operator/trust" || path === "/api/trust")
    ) {
      const trust = await getOperatorTrust(
      ...
      sendJson(res, 200, {
        score: Math.max(
          ...
          Math.min(
            100,
            100 - trust.pendingApprovals * 5 - trust.lowConfidenceMemories,
          ),
        ),
        ...
        lanes: [
          {
            ...
            positioning:
              "control plane for journeys, approvals, and persistent operator memory",
            status: "active",
            summary:
              "Primary operator lane for governed engineering execution.",
          },
          {
            key: "debo_unchained",
            name: "DEBO Unchained",
            ...
            positioning:
              "offensive hack-team package for authorized engagements",
            status: trust.unchainedReady ? "active" : "pilot",
            summary:
              "Reserved lane for NATT-led offensive operations with human review.",
          },
        ],
      });
      return true;
    ...
    const approveMatch =
      method === "POST"
        ? path.match(/^\/api\/approvals\/([^/]+)\/approve$/)
        : null;
    if (approveMatch) {
      const body = await readJson<{
        approvedBy?: string;
    ...
    const rejectMatch =
      method === "POST"
        ? path.match(/^\/api\/approvals\/([^/]+)\/reject$/)
        : null;
    if (rejectMatch) {
      const body = await readJson<{
        rejectedBy?: string;
    ...
    const teachMatch =
      method === "POST"
        ? path.match(/^\/api\/approvals\/([^/]+)\/teach$/)
        : null;
    if (teachMatch) {
      const body = await readJson<{
        taughtBy?: string;
    ...
    if (
      method === "GET" &&
      (path === "/api/policy/show" || path === "/api/policy")
    ) {
